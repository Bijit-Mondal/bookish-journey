...
function ShowResults(response){
  ...
  if(json.items==null||json.items.length==null){
    ...
    let template = '<div class="card"><img src="THUMBNAIL_URL" draggable="false"></div>'+
                      '<p class="card_para">TITLE</p>'+
                      '<div class="button">'+
                      '<button class="btn" onclick="DownloadAudio(`VIDEO_ID`,`TITLE`)">Download Audio</button>'+
                      '<button class="btn" onclick="DownloadVideo(`VIDEO_ID`,`TITLE`)">Download Video</button>'+
                      '</div>';
  ...
  let vid = '21212';
  let container = this.document.getElementsByClassName('result')[0];
